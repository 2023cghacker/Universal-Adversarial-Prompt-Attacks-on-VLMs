...
class QwenAdversarialPatch(QwenAdversarialBase):
    ...
    def train_patch(self, patch_size=(50, 50), position=(0, 0)):
        ...
        H, W, _= base_tensor.shape
        ph, pw = patch_size
        y, x = position
        assert y+ph <= H and x+pw <= W, "Patch 超出了原图范围! ph={ph}, pw={pw}, H={H}, W={W}, x={x}, y={y}"

        # 初始化 patch``
        patch = torch.randn((ph, pw,3), device=self.device, requires_grad=True)
        ...
        for step in range(self.steps):
            ...
            if (step+1) % 1 == 0:
                ...
                if (step + 1) % 10 == 0:
                    ...
                    T.ToPILImage()(patched_image.permute(2,0,1).cpu()).save(img_path)
                    # T.ToPILImage()(patched_image.to(torch.uint8).permute(2,0,1).cpu()).save(img_path)
...
if __name__ == "__main__":
    ...
    adv_patch.train_patch(patch_size=(440, 440), position=(20, 30))
